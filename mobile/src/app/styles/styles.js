export const primaryColour = '#30415d';
export const lightPrimaryColour = '#8eaebd';
export const secondaryColour = '#cf6766';
export const backgroundColour = '#031424';
export const editButtonColour = '#0288D1';
export const deleteButtonColour = '#D32F2F';
export const addButtonColour = '#2E7D32';
export const iconColour = '#FFF';
export const textColour = '#FFF';

export const fontFamilyLight = 'Montserrat-Light';
export const fontFamilyRegular = 'Montserrat-Regular';
export const fontFamilyMedium = 'Montserrat-Medium';
export const fontFamilyBold = 'Montserrat-Bold';

export const fontSizeLg = 28;